...
If multiple suffixes exist (like `C`, `cc`, `cpp` and `h` for C++ files),
...
Since version 4.6 highlight can be invoked as a long running process to process
files using stdin and stdout. This might result in better performance, e.g.,
...
or syntax=<syntax_suffix> to set the syntax to use directly (e.g., csharp).
...
are generally easy for most clients to send, e.g., 0x07 ('\a') for bell. It is
nearly impossible to send several characters above 128 in Windows through most
terminal emulators.
...
options line. You also cannot send the EOF char to a new line as it is used as a
...
If multiple suffixes exist, list them in {filetypes_conf}.
...
Regular expressions are evaluated in their order within Keywords. If a regex
